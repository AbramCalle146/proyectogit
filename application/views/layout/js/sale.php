...
<script>
    ...
    $(".list-product tr").each(function() {
        // Extraer el importe de la celda correspondiente
        let importe = Number($(this).find("td:eq(5)").text());
        console.log("Importe de la fila:", importe); // Depuración
    ...
    console.log("Subtotal antes de mostrar:", subtotal); // Depuración

    $("#subtotal").val(subtotal.toFixed(2)); // Mostrar subtotal con dos decimales

    let porcentajeIgv = Number($("#voucherIgv").val()) || 0;
    let igv = subtotal * (porcentajeIgv / 100);
    $("#igv").val(igv.toFixed(2)); // Mostrar IGV con dos decimales

    let descuento = Number($("#discount").val()) || 0;
    let total = (subtotal - descuento) + igv;

    // Asegúrate de que total es un número válido
    if (isNaN(total)) {
        total = 0; // O asigna un valor predeterminado
    }

    $("#total").val(total.toFixed(2)); // Mostrar total con dos decimales

    console.log("Total después de calcular:", total); // Depuración
}
